...
test_root="$(cd "$(dirname "$0")/.." && pwd)"
{
  ...
  sed '/^BEGIN;$/d; /^COMMIT;$/d' \
    ...
    "$test_root/migrations/20260919115010_share_stable_assignments.sql"
  cat "$test_root/scripts/test-startlijst-rls.sql" "$test_root/scripts/test-score-access.sql" "$test_root/scripts/test-stal-access.sql"
  printf '\nROLLBACK;\n'
