...
export function styleRules(klasse) {
 ...
 return { id: c.styleTrailRuleSetId, minObstacles: c.min, maxObstacles: c.max,
  generalPoints: ['junior', 'yr'].includes(c.code) ? null : ['we0', 'we1'].includes(c.code) ? STYLE_GENERAL_BASIC : STYLE_GENERAL_ADVANCED,
...
export function styleMaximum(klasse, obstacleCount) {
 ...
 if (!Number.isInteger(obstacleCount) || obstacleCount < rule.minObstacles || obstacleCount > rule.maxObstacles) throw new Error(`Stijltrail vereist ${rule.minObstacles}–${rule.maxObstacles} beoordelingen.`);
