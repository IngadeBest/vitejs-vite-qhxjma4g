...
export default function InschrijfFormulier({ initialWedstrijdId = '', onSaved = null }) {
  ...
  async function onSubmit(e) {
    ...
    if (REGISTRATION_MAINTENANCE) {
      setMsg("Inschrijven tijdelijk gesloten: " + REGISTRATION_MAINTENANCE_MESSAGE);
      return;
    }
    setBusy(true);
    setMsg("");
  ...
  return (
    <div style={{ maxWidth: 760, margin: "24px auto" }}>
      ...
      {REGISTRATION_MAINTENANCE && (
        <Alert type="info">
          <strong>Inschrijven tijdelijk gesloten.</strong> {REGISTRATION_MAINTENANCE_MESSAGE}
        </Alert>
      )}
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: "10px 12px", alignItems:"center" }}>
        ...
        <select id="wedstrijd_select" value={form.wedstrijd_id} onChange={(e)=>setForm(s=>({...s, wedstrijd_id:e.target.value}))} disabled={REGISTRATION_MAINTENANCE || loading}>
        ...
        <select id="klasse_select" value={form.klasse} onChange={(e)=>setForm(s=>({...s, klasse:e.target.value}))} disabled={REGISTRATION_MAINTENANCE}>
        ...
        {(() => {
          ...
          return (
            <>
              ...
              <select id="rubriek_select" value={form.rubriek} onChange={(e)=>setForm(s=>({...s, rubriek:e.target.value}))} disabled={REGISTRATION_MAINTENANCE}>
        ...
        <Input id="ruiter_input" ref={firstInputRef} value={form.ruiter} onChange={(e)=>setForm(s=>({...s, ruiter:e.target.value}))} disabled={REGISTRATION_MAINTENANCE} />

        <label htmlFor="paard_input">Paard*</label>
        <Input id="paard_input" value={form.paard} onChange={(e)=>setForm(s=>({...s, paard:e.target.value}))} disabled={REGISTRATION_MAINTENANCE} />

        <label htmlFor="email_input">E-mail</label>
        <Input id="email_input" type="email" value={form.email} onChange={(e)=>setForm(s=>({...s, email:e.target.value}))} disabled={REGISTRATION_MAINTENANCE} />

        <label htmlFor="telefoon_input">Telefoon</label>
        <Input id="telefoon_input" value={form.telefoon} onChange={(e)=>setForm(s=>({...s, telefoon:e.target.value}))} disabled={REGISTRATION_MAINTENANCE} />

        <label htmlFor="omroeper_input">Tekst voor omroeper</label>
        <textarea id="omroeper_input" rows={3} value={form.omroeper} onChange={(e)=>setForm(s=>({...s, omroeper:e.target.value}))} className="border rounded px-2 py-1 w-full" disabled={REGISTRATION_MAINTENANCE} />

        <label htmlFor="opmerkingen_input">Opmerkingen</label>
        <textarea id="opmerkingen_input" rows={3} value={form.opmerkingen} onChange={(e)=>setForm(s=>({...s, opmerkingen:e.target.value}))} className="border rounded px-2 py-1 w-full" disabled={REGISTRATION_MAINTENANCE} />
