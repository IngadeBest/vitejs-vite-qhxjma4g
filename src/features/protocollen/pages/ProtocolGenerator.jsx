...
/* PDF Layout Constanten - Geoptimaliseerd voor schrijfruimte en uitlijning */
const BLUE = [16, 39, 84];
...
function obstaclesTable(doc, items, startY) {
  // Check of we speedtrail data hebben (arrays met strafregels) of simpele strings (stijltrail)
...
function generalPointsTable(doc, punten, startY, startIndex = 1) {
  autoTable(doc, {
    ...
    columnStyles: {
      0: { cellWidth: COL_WIDTHS.NUM,  halign: "center" },
      1: { cellWidth: COL_WIDTHS.LETTER + COL_WIDTHS.EXERCISE }, // Uitlijnen met hoofdtabel
      2: { cellWidth: COL_WIDTHS.HEEL,    halign: "center" },
      3: { cellWidth: COL_WIDTHS.HALF, halign: "center" },
      4: { cellWidth: COL_WIDTHS.NOTE },
    },
...
function totalsBox(doc, startY, maxPoints = null, extraLabel = null, showPuntenaftrek = true, isDressuur = false, isSpeed = false) {
  ...
  if (isSpeed) {
    ...
    colStyles = {
      ...
      1: { cellWidth: COL_WIDTHS_SPEED.SCORE, halign: "center" }, // Lijn loopt door vanuit 'Straf'
      2: { cellWidth: COL_WIDTHS_SPEED.NOTE }                     // Lijn loopt door vanuit 'Opmerking'
    };
  ...
  if (p.onderdeel === "dressuur") {
    ...
    autoTable(doc, {
      ...
      styles: { 
        ...
        valign: "top",
        overflow: 'linebreak',
        minCellHeight: 70  // Veel schrijfruimte
      },
      headStyles: { 
        fillColor: HEADER_COLOR, 
        textColor: 0, 
...
export default function ProtocolGenerator() {
  ...
  async function loadDeelnemersFromDB() {
    ...
    try {
      const klasseMap = { 'we0':'WE0', 'we1':'WE1', 'we2':'WE2', 'we2p':'WE2+', 'we2+':'WE2+', 'we3':'WE3', 'we4':'WE4', 'yr':'YR', 'junior':'Junioren', 'junioren':'Junioren' };
      const normalizedKlasse = klasseMap[config.klasse.toLowerCase()] || config.klasse.toUpperCase();
