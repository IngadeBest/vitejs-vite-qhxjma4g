...
function countScoringLines(items) { return (items || []).reduce((acc) => acc + 1, 0); }
function obstaclesTable(doc, obstakels, startY) {
  autoTable(doc, {
    ...
    didParseCell: (d) => { if (d.section === "body" && d.column.index === 1) d.cell.styles.valign = "top"; },
...
function totalsBox(doc, startY, maxPoints = null) {
  ...
  autoTable(doc, {
    startY, head: [],
    body: [["Subtotaal", ""], ["Puntenaftrek en reden", ""], [totalLabel, ""]],
    styles: { fontSize: 10, cellPadding: 6, lineColor: BORDER, lineWidth: 0.2 },
    theme: "grid", margin: MARGIN, columnStyles: { 0: { cellWidth: 180, fontStyle: "bold" }, 1: { cellWidth: "auto" } },
  });
  return doc.lastAutoTable.finalY;
}
...
export default function ProtocolGenerator() {
  ...
  const [config, setConfig] = useState({ klasse: "", wedstrijd: "", datum: new Date().toISOString().split("T")[0], jury: "" });
  ...
  const minMax = useMemo(() => selectedKlasse ? ({ min: selectedKlasse.min, max: selectedKlasse.max }) : { min:0, max:0 }, [selectedKlasse]);
  const catalogList = useMemo(() => selectedKlasse ? (obstakelsCatalog?.[selectedKlasse.labelKey] ?? []).slice() : [], [selectedKlasse]);

  const addSingle = (naam) => setObstakels(prev => prev.length >= minMax.max ? prev : [...prev, { type: "single", naam }]);
  const addGroup  = (items) => {
    const clean = items.map(s=>s.trim()).filter(Boolean);
    if (!clean.length) return;
    setObstakels(prev => prev.length >= minMax.max ? prev : [...prev, { type: "group", items: clean }]);
  };
  const removeAt  = (i) => setObstakels(prev => prev.filter((_, idx) => idx !== i));
  ...
  const onDrop      = (i) => (e) => {
    ...
    setObstakels(prev => { const next=[...prev]; const [m]=next.splice(from,1); next.splice(i,0,m); return next; });
  ...
  const handleBulkAdd = () => {
    ...
    setObstakels(next); setBulkText("");
  };

  const loadTemplate = () => {
    if (!selectedKlasse) return;
    const list = templates?.[selectedKlasse.labelKey] && Array.isArray(templates[selectedKlasse.labelKey]) ? templates[selectedKlasse.labelKey] : [];
  ...
  function importFromStorage() {
    try {
      ...
      if (!csv.trim()) return alert("Geen startlijst gevonden in opslag.");
      const { headers, rows } = csvToRows(csv);
      const req = ["ruiter", "paard", "startnummer"];
      const low = headers.map((h) => h.toLowerCase().trim());
      if (!req.every((x) => low.includes(x))) return alert("Opgeslagen startlijst mist verplichte kolommen.");
      const idx = Object.fromEntries(low.map((h, i) => [h, i]));
      const norm = rows.map(row => ({
        ruiter: row[headers[idx["ruiter"]]] || row["ruiter"] || "",
        paard: row[headers[idx["paard"]]] || row["paard"] || "",
  ...
  const handleBulkAdd = () => {
    ...
    for (const line of lines) {
      if (next.length >= minMax.max) break;
      if (line.includes("|")) { const items = line.split("|").map(s=>s.trim()).filter(Boolean); next.push({ type:"group", items }); }
      else { next.push({ type:"single", naam: line }); }
    }
    setObstakels(next); setBulkText("");
  };
    ...
    <>
      ...
      <div style={{ maxWidth: 1200, margin: "24px auto" }}>
          ...
          <div>
            ...
            <div style={{ marginTop: 16 }}>
              <div style={{ fontWeight: 600, marginBottom: 6 }}>
                Gekozen volgorde ({obstakels.length}) {obstakels.length < minMax.min ? \`— min \${minMax.min}\` : ""} {obstakels.length > minMax.max ? \`— max \${minMax.max}\` : ""}
              </div>
        ...
        <div style={{ marginTop: 24, display: "flex", gap: 10 }}>
          ...
          <button onClick={() => setStap(3)} disabled={obstakels.length < minMax.min || obstakels.length > minMax.max || !csvRows.length}>Volgende: Overzicht & PDF</button>
        </div>
      </div>
    </>
  );
